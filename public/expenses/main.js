...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const expense = await axios.post(
      "http://3.27.95.152:3000/expenses/add-expense",
      data,
      {
...
expenseList.addEventListener("click", async (e) => {
  try {
    if (e.target.classList.contains("delete")) {
      ...
      await axios.delete(`http://3.27.95.152:3000/expenses/delete/${id}`, {
        headers: { token: localStorage.getItem("token") },
      });
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    const res = await axios.get(
      `http://3.27.95.152:3000/expenses/get-Paginated-expenses/?pageNo=1&limit=${localStorage.getItem(
        "listRows"
      )}`,
      {
        headers: { token: localStorage.getItem("token") },
...
premiumBtn.addEventListener("click", async () => {
  try {
    const result = await axios.get(
      "http://3.27.95.152:3000/purchase/buy-premium",
      {
        headers: { token: localStorage.getItem("token") },
      }
    );
    ...
    const options = {
      ...
      handler: async (response) => {
        const update = await axios.post(
          "http://3.27.95.152:3000/purchase/update-premium",
          {
...
leadershipBtn.addEventListener("click", async () => {
  if (leadershipList.style.display == "none") {
    leadershipList.style.display = "display: inline-block";
...
async function download() {
  try {
    ...
    const response = await axios.get(
      "http://3.27.95.152:3000/expenses/download",
      {
        headers: { token: localStorage.getItem("token") },
      }
    );
...
expenseListPaginate.addEventListener("click", async (e) => {
  if (e.target.classList.contains("paginateButtonStyle")) {
    ...
    const res = await axios.get(
      `http://3.27.95.152:3000/expenses/get-Paginated-expenses/?pageNo=${pageNo}&limit=${localStorage.getItem(
        "listRows"
      )}`,
      {
        headers: { token: localStorage.getItem("token") },
